...
export function ControlsHint({ isMobile }: ControlsHintProps) {
  if (isMobile) {
    return (
      <div
        ...
        "
      >
        💬 Ketuk ikon chat atau tekan{" "}
        <kbd className="mx-1 px-3 py-1 bg-white/20 rounded-lg text-base font-bold">
          T
        </kbd>{" "}
        untuk chat
      </div>
  ...
  // DESKTOP (biarkan seperti sekarang, sudah oke)
  return (
    <div className="absolute bottom-5 right-5 text-white bg-black/80 px-4 py-3 rounded-lg text-sm space-y-1 max-w-xs hidden">
